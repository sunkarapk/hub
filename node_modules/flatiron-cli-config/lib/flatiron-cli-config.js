...
cliConfig.attach = function (options) {
  ...
  app.commands['conf'] = app.commands['conf'] || {};
  app.commands['conf'] = common.mixin(app.commands['conf'], cliConfig.commands);
};
...
cliConfig.detach = function () {
  ...
  Object.keys(app.commands['conf']).forEach(function (method) {
    if (cliConfig.commands[method]) {
      delete app.commands['conf'][method];
    }
